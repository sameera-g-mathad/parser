  ...
  */
  ...
  /* 
    ...
    original_name = TEXT NOT NULL, [Used to send in email]
  ...
  */
  ...
  /*
   Schema(Context):
     This table consists each running context and knowledge sources,
     i.e., the page numbers used for each prompt answered by llm. This
     table exists so that we can retrieve previous running contexts
     and not lose context in the ui.
     id =  UUID (Primary Key), by default generated by postgres,
     conversation_id = UUID (Foreign Key), references `conversations`.
     page_numbers = Number[], Page numbers used as the knowledge source.
     running_context = Running context used to send llm the standalone question
  */ 
    CREATE TABLE IF NOT EXISTS CONTEXT(
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      conversation_id UUID REFERENCES CONVERSATIONS(ID) ON DELETE CASCADE NOT NULL,
      page_numbers INTEGER[],
      running_context TEXT NOT NULL,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
